...
function LoginPageContent() {
  ...
  // Wake the Render free-tier API while the user types credentials.
  useEffect(() => {
    const controller = new AbortController();
    fetch(`${API_BASE_URL}/api/health`, { signal: controller.signal, cache: "no-store" }).catch(
      () => {}
    );
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => setSlowHint(true), 2500);
        ...
        <main className="relative flex min-h-dvh flex-col">
          ...
          <div className="relative z-10 -mt-5 flex flex-1 flex-col rounded-t-[1.75rem] theme-sheet px-5 pb-8 pt-6 shadow-[0_-12px_40px_rgba(42,24,16,0.18)] sm:px-8 md:px-12 lg:mt-0 lg:rounded-none lg:bg-transparent lg:px-16 lg:pb-10 lg:pt-8 lg:shadow-none xl:px-20">
            ...
            <div className="mx-auto flex w-full max-w-[26rem] flex-1 flex-col justify-center lg:py-6">
              ...
              <form onSubmit={handleLogin} className="flex flex-col gap-4 sm:gap-5">
                ...
                {slowHint && submitting && (
                  <p className="text-center font-paragraph text-xs text-dark-brown/55">
                    Server is waking up — usually takes a few seconds on free hosting.
                  </p>
                )}
              </form>
              ...
              <p className="mt-6 pb-[env(safe-area-inset-bottom)] text-center font-paragraph text-sm text-dark-brown/70 sm:mt-8 sm:text-base">
